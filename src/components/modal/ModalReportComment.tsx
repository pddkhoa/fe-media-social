...
type ModalReportCommentProps = {
    data: Comment;
};

const ModalReportComment: FC<ModalReportCommentProps> = ({ data }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const { body } = await UserServices.reportComment(report);
                if (body?.success) {
    ...
    useEffect(() => {
        if (valueReport.length > 0) {
            setValue(valueReport[0]._id); // Chọn giá trị đầu tiên từ mảng valueReport
        }
    }, [valueReport]);
