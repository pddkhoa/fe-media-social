...
import { Title, Button, Loader, Avatar } from "rizzui";

type ProfileHeaderProps = {
    isView?: boolean;
};

export default function ProfileHeader({ isView }: ProfileHeaderProps) {
    ...
    const fetchData = useCallback(async () => {
        try {
            setIsLoading(true);
            const { body } = await UserServices.getWallDetail(user._id);
            if (body?.success) {
                setIsLoading(false);
            ...
            {isLoading ? (
                ...
                <div className="mx-auto w-full max-w-[1294px] @container  sm:flex sm:justify-between">
                    ...
                    <div className="grid grid-cols-2 pt-3 @3xl:pt-4">
                        {isView ? (
                            <Button className="col-span-2">View Profile</Button>
                        ) : (
                            <>
                                <Button
                                    variant="outline"
