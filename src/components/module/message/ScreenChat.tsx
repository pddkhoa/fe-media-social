...
export const ScreenChat: FC<ScreenChatProps> = ({
    ...
    const renderMessages = () => {
        ...
        return messages.map((item: any) =>
            item?.user?._id === user?.user?._id ? (
                <MeChat data={item} key={item?._id} setIsDelete={setIsDelete} />
            ) : (
    ...
    const renderInputSection = () => (
        <div className="flex flex-row items-center h-16 rounded-xl bg-white w-full px-4">
                ...
                <Button
                    onClick={() => {
                        openModal({
                            view: (
                                <UploadModal
                                    handleUploadImage={handleUploadIamgeMessage}
            ...
            <div className="ml-4">
                <Button
                    disabled={contentMessage === "" && imageUrl === ""}
    ...
    const renderChatHeader = () => {
        const avatarUrl = dataChat?.isGroup
            ? dataChat.avatar.url
            : dataChat?.userReceived.avatar.url;
        const chatName = dataChat?.isGroup
            ? dataChat.chatName
    ...
    return (
        <div className="pl-8 h-[49rem] w-full sm:h-[50vh] md:h-[60vh] lg:h-[70vh] xl:h-[80vh]">
            {renderChatHeader()}
            <div className="flex flex-col h-full justify-between">
                <SimpleBar
                    scrollableNodeProps={{ ref: messagesContainerRef }}
                    className="flex flex-col h-full rounded bg-gray-200 overflow-y-auto"
                >
